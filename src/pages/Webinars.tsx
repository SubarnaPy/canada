...
const Webinars = () => {
  ...
  const fetchWebinars = async () => {
    try {
      ...
      // Fetch registration counts for each webinar using the secure function
      if (data) {
        const counts: Record<string, number> = {};
        for (const webinar of data) {
          const { data: countData, error: countError } = await supabase
            .rpc("get_webinar_registration_count", { webinar_id: webinar.id });
          
          if (!countError && countData !== null) {
            counts[webinar.id] = countData;
          } else {
            counts[webinar.id] = 0;
          }
        }
        ...
        <div className="container mx-auto px-4">
          {isLoading ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3].map((i) => (
                <Card key={i} className="p-6 bg-gradient-to-br from-[#181A20] to-[#0B0E11] border-[#F0B90B]/10 animate-pulse">
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-col">
                      ...
                      <Button
                        ...
                        disabled={isFull}
                        className="w-full bg-[#F0B90B] hover:bg-[#F3BA2F] text-[#0B0E11] disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isFull ? (
                          <>
